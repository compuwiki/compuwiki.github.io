...
_ Integrated: database in the same project like [Wikipedia](https://en.wikipedia.org/wiki/Main_Page) or [ArchWiki](https://wiki.archlinux.org/) that use MediaWiki as CMS.
_ External: database managed by an external application, by the same team or an external tea. Like [Hyprland wiki](https://wiki.hyprland.org/) that uses GitHub for hosting the website acting as database to manage collabaratos and commits.
